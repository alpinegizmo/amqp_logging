...
You can pass in a formatter if you want to alter all of the log lines. For example, if you want syslog compatible logging:

	syslog_formatter = lambda do |msg|
	  LogjamLogger::Formatter.new.format(msg)
	end

	config.logger = AMQPLogging::Logger.new(config.paths.log.to_a.first, :formatter => syslog_formatter)
